...
/* AD Type for vendor-specific data (must match broadcast_source) */
...
/* ===== PA data receive (Vendor-Specific AD element extraction) ===== */
...
static bool pa_data_parse_cb(struct bt_data *data, void *user_data)
{
	...
	if (changed) {
		LOG_INF("PA custom data received (%u bytes)", data->data_len);
...
static int zbus_subscribers_create(void)
{
	...
	if (ret) {
		LOG_ERR("Failed to create bt_mgmt_msg thread");
		return ret;
	}
...
/* ===== streamctrl interface (required by audio_system) ===== */
...
int main(void)
{
	...
	/* Register PA data receive callback for extracting VS AD data */
	...
	LOG_INF("Broadcast sink started, waiting for PA data...");
