...
static void encoder_thread(void *arg1, void *arg2, void *arg3)
{
	...
	while (1) {
		...
		/* Wait for audio data, but check periodically for test tone activation */
		while (true) {
			ret = k_msgq_get(&audio_q_rx, (void *)&audio_block,
					 K_MSEC(CONFIG_AUDIO_FRAME_DURATION_US / 1000));
			if (ret == 0) {
				break;
			}
			if (test_tone_size) {
		...
		if (audio_block != NULL) {
			/* Audio data received from USB/I2S: collect full frame */
			...
			if (sw_codec_cfg.encoder.enabled && test_tone_size) {
				/* Test tone overwrites audio source data */
			...
			/* Standalone test tone: no audio source, generate directly */
			uint32_t num_bytes = 0;
			char tmp[FRAME_SIZE_BYTES / 2];
			...
			net_buf_add(audio_frame, FRAME_SIZE_BYTES);
			ret = pscm_copy_pad(tmp, FRAME_SIZE_BYTES / 2,
					    CONFIG_AUDIO_BIT_DEPTH_BITS, audio_frame->data,
					    &num_bytes);
			ERR_CHK(ret);
			...
			meta->data_len_us = CONFIG_AUDIO_FRAME_DURATION_US;
			...
			while (k_msgq_get(&audio_q_rx, (void *)&stale, K_NO_WAIT) == 0) {
				net_buf_unref(stale);
			}

			k_sleep(K_USEC(CONFIG_AUDIO_FRAME_DURATION_US));
		}
